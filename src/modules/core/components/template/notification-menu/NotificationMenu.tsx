...
const NotificationToggle: React.FC<NotificationToggleProps> = forwardRef<
  HTMLButtonElement,
...
export const NotificationMenu = () => {
  ...
  useEffect(() => {
    if (lastMessage !== null) {
      ...
      const notificationFound = notificationHistory.find(
        (message) =>
          message.NotificationId === currentNotification.NotificationId
      );
  ...
  useEffect(() => {
    const unReadCount = notificationHistory.filter(
        ...
        <ScrollArea h={200}>
          {connectionStatus === 'Closed' && (
            <Alert severity='error' className='m-2'>
              <span>Unable to display notifications</span>
            </Alert>
          )}
                ...
                notificationHistory.map((message, index) => {
                  return (
                    <List.Item
                      ...
                      onClick={() => marksAsReadHandler(message.NotificationId)}
                    >
